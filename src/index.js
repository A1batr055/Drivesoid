...
function getStatus() {
  try {
    ...
    return { snapshot_at: state.snapshot_at, snapshot_age_ms, stale, error: stale, base: state.base, display: d, prev: state.prev_display || null, groups, whim, sleep: state.sleep || null };
  } catch {
    return { error: true, stale: true, snapshot_age_ms: null };
  }
}
